...
def redact_mcp_command(command: Sequence[str]) -> list[str]:
    ...
    for part in command:
        ...
        if lowered_option in _SECRET_OPTION_NAMES or (
            lowered_option.startswith("--") and any(fragment in lowered_option for fragment in _SECRET_OPTION_FRAGMENTS)
        ):
            if separator:
                redacted.append(f"{option_name}=<redacted>")
            else:
...
def _looks_like_secret_env(name: str) -> bool:
    ...
    return any(upper_name == suffix or upper_name.endswith(f"_{suffix}") for suffix in _SECRET_ENV_SUFFIXES)
