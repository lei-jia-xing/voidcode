...
## NOTES
- Top-level execution is limited to `leader`; supported delegated child presets are `advisor`, `explore`, `researcher`, `worker`, and `product` (read-only plan agent delegated via `task`, handing the plan back with `submit_result`).
