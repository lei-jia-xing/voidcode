...
def _continuation_loop_fields(loop: ContinuationLoopState) -> list[tuple[str, object]]:
    fields: list[tuple[str, object]] = [
        ("id", loop.loop.id),
        ("status", loop.status),
        ...
        ("completion_promise", repr(loop.completion_promise)),
    ...
    if loop.finished_at is not None:
        fields.append(("finished_at", loop.finished_at))
    if loop.cancel_requested_at is not None:
        fields.append(("cancel_requested_at", loop.cancel_requested_at))
    if loop.error is not None:
        fields.append(("error", loop.error))
    fields.append(("prompt", repr(loop.prompt)))
    return fields


def _continuation_loop_payload(loop: ContinuationLoopState) -> dict[str, object]:
    return {
...
def _continuation_loop_summary_payload(
    loop: StoredContinuationLoopSummary,
) -> dict[str, object]:
    return {
        "loop_id": loop.loop.id,
        "status": loop.status,
...
def _format_continuation_loop_state(loop: ContinuationLoopState) -> str:
    return _format_named_record("LOOP", _continuation_loop_fields(loop))


def _format_continuation_loop_summary(loop: StoredContinuationLoopSummary) -> str:
    fields: list[tuple[str, object]] = [
        ("id", loop.loop.id),
        ("status", loop.status),
        ("session_id", loop.session_id),
        ...
        ("intensive", loop.intensive),
        ("updated_at", loop.updated_at),
        ("prompt", repr(loop.prompt)),
    ]
    if loop.error is not None:
        fields.append(("error", loop.error))
    return _format_named_record("LOOP", fields)
...
def _handle_loops_start_command(args: argparse.Namespace) -> int:
    workspace = cast(Path, args.workspace)
    json_output = cast(bool, getattr(args, "json", False))
    runtime = VoidCodeRuntime(workspace=workspace)
    try:
        try:
            loop = runtime.start_continuation_loop(
                prompt=cast(str, args.prompt),
                session_id=cast(str | None, getattr(args, "session_id", None)),
                completion_promise=cast(str, args.completion_promise),
                max_iterations=cast(int, args.max_iterations),
                intensive=cast(bool, getattr(args, "intensive", False)),
                strategy=cast(ContinuationLoopStrategy, args.strategy),
            )
        except ValueError as exc:
            raise SystemExit(f"error: {exc}") from None
    finally:
    ...
    if json_output:
        print_json({"workspace": str(workspace), "loop": _continuation_loop_payload(loop)})
        return 0
    print(_format_continuation_loop_state(loop))
    return 0


def _handle_loops_status_command(args: argparse.Namespace) -> int:
    workspace = cast(Path, args.workspace)
    json_output = cast(bool, getattr(args, "json", False))
    runtime = VoidCodeRuntime(workspace=workspace)
    try:
        try:
            loop = runtime.load_continuation_loop(cast(str, args.loop_id))
        except ValueError as exc:
            raise SystemExit(f"error: {exc}") from None
    finally:
        _close_runtime(runtime)
...
def _handle_loops_cancel_command(args: argparse.Namespace) -> int:
    workspace = cast(Path, args.workspace)
    json_output = cast(bool, getattr(args, "json", False))
    runtime = VoidCodeRuntime(workspace=workspace)
    try:
        try:
            loop = runtime.cancel_continuation_loop(cast(str, args.loop_id))
        except ValueError as exc:
            raise SystemExit(f"error: {exc}") from None
    finally:
        _close_runtime(runtime)
...
def _handle_loops_list_command(args: argparse.Namespace) -> int:
    workspace = cast(Path, args.workspace)
    json_output = cast(bool, getattr(args, "json", False))
    runtime = VoidCodeRuntime(workspace=workspace)
    try:
        loops = runtime.list_continuation_loops()
    finally:
        _close_runtime(runtime)
    ...
    if json_output:
        print_json(
            {
                "workspace": str(workspace),
                "loops": [_continuation_loop_summary_payload(loop) for loop in loops],
            }
        )
        return 0
    for loop in loops:
        print(_format_continuation_loop_summary(loop))
    return 0
...
@root_cli.group(help="Inspect runtime-owned continuation loops.")
def loops() -> None:
    pass


@loops.command(name="start", help="Start a persisted continuation loop state.")
@click.argument("prompt")
@_workspace_option("Workspace root used to resolve the local session database.")
@click.option("--session-id")
@click.option("--completion-promise", default="DONE", show_default=True)
@click.option("--max-iterations", type=int, default=100, show_default=True)
@click.option("--intensive", is_flag=True, help="Mark this loop for higher-intensity continuation.")
@click.option(
    "--strategy",
    type=click.Choice(["continue", "reset"]),
    default="continue",
    show_default=True,
)
@_json_option("Output continuation loop state as JSON.")
def loops_start(
    prompt: str,
    workspace: Path,
    session_id: str | None,
...
@loops.command(name="status", help="Show continuation loop state.")
@click.argument("loop_id")
@_workspace_option("Workspace root used to resolve the local session database.")
@_json_option("Output continuation loop state as JSON.")
def loops_status(loop_id: str, workspace: Path, json_output: bool) -> int:
    return _invoke_handler_from_click(
        _handle_loops_status_command,
        _build_click_command_context(
            command="loops",
            subcommand="status",
            loop_id=loop_id,
...
@loops.command(name="cancel", help="Cancel an active continuation loop.")
@click.argument("loop_id")
@_workspace_option("Workspace root used to resolve the local session database.")
@_json_option("Output cancelled continuation loop state as JSON.")
def loops_cancel(loop_id: str, workspace: Path, json_output: bool) -> int:
    return _invoke_handler_from_click(
        _handle_loops_cancel_command,
        _build_click_command_context(
            command="loops",
            subcommand="cancel",
            loop_id=loop_id,
...
@loops.command(name="list", help="List continuation loops.")
@_workspace_option("Workspace root used to resolve the local session database.")
@_json_option("Output continuation loop summaries as JSON.")
def loops_list(workspace: Path, json_output: bool) -> int:
    return _invoke_handler_from_click(
        _handle_loops_list_command,
        _build_click_command_context(
            command="loops",
            subcommand="list",
            workspace=workspace,
