...
Use this skill when asked to review, verify, QA, or validate implementation work. It is a local VoidCode adaptation, not an OhMyOpenAgent orchestration copy. It must only describe capabilities VoidCode actually supports: top-level `leader`, runtime-owned delegated child presets `advisor`, `explore`, `researcher`, `worker`, and read-only planning child `product`, runtime tools, local files, git history, and configured MCP descriptors such as `context7`, `websearch`, `grep_app`, and `playwright`.
