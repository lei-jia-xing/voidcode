...
from voidcode.tools import AstGrepTool, ToolCall, WebFetchTool, WebSearchTool


def test_ast_grep_honors_runtime_timeout(tmp_path: Path) -> None:
    ...
    with patch("subprocess.run", side_effect=TimeoutError("boom")):
        result = tool.invoke_with_runtime_timeout(
            ToolCall(tool_name="ast_grep", arguments={"mode": "search", "pattern": "print($X)", "path": "sample.py"}),
            workspace=tmp_path,
            timeout_seconds=1,
        )
