...
def test_rulebook_transform_reuses_reactive_file_rule_provider(tmp_path: Path) -> None:
    ...
    assert "rule://deploy" in contents


def test_rule_uri_success_unknown_traversal_and_bounded_read(tmp_path: Path) -> None:
    _write_rules(tmp_path)
    result = read_rule_uri("rule://deploy", workspace=tmp_path, limit=1)
    ...
    with pytest.raises(ValueError, match="unknown rule"):
        read_rule_uri("rule://missing", workspace=tmp_path)
    with pytest.raises(ValueError, match="invalid rule URI"):
        read_rule_uri("rule://%2e%2e%2fsafety", workspace=tmp_path)
    with pytest.raises(ValueError, match="positive"):
        read_rule_uri("rule://deploy", workspace=tmp_path, offset=0)
...
def test_read_tool_serves_rule_uri_and_never_falls_through_to_external_path(tmp_path: Path) -> None:
    _write_rules(tmp_path)
    result = ReadTool().invoke(ToolCall(tool_name="read", arguments={"path": "rule://safety"}), workspace=tmp_path)
