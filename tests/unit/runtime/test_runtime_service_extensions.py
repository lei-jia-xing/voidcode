...
from voidcode.runtime.plan import PlanArtifact, PlanStep
...
def test_runtime_plan_first_emits_plan_created_and_persists_artifact_metadata(
    tmp_path: Path,
) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
    ...
    chunks = list(
        runtime.run_stream(
            RuntimeRequest(prompt="read sample.txt", session_id="plan-first-session")
        )
    )
    response = runtime.resume("plan-first-session")

    plan_events = [event for event in response.events if event.event_type == RUNTIME_PLAN_CREATED]
    output_chunk_index = next(index for index, chunk in enumerate(chunks) if chunk.kind == "output")
    plan_chunk_index = next(
        index
        for index, chunk in enumerate(chunks)
        if chunk.event is not None and chunk.event.event_type == RUNTIME_PLAN_CREATED
    )

    assert response.session.status == "completed"
    assert len(plan_events) == 1
    assert plan_chunk_index < output_chunk_index
    assert plan_events[0].payload == {
        "kind": "leader.plan_first",
        "version": 1,
        "step_count": 1,
        "acceptance_criteria_count": 2,
        "execution_hint_count": 2,
    }
    plan_artifact = cast(dict[str, object], response.session.metadata["plan_artifact"])
    assert plan_artifact["version"] == 1
    assert plan_artifact["kind"] == "leader.plan_first"
    assert plan_artifact["acceptance_criteria"] == [
        "Execution should stay aligned with the requested task.",
        "Runtime validations must pass before graph/tool execution begins.",
    ]
    assert plan_artifact["execution_hints"] == [
    ...
    assert cast(dict[str, object], plan_artifact["metadata"])["agent_preset"] == "leader"
    assert cast(dict[str, object], plan_artifact["metadata"])["prompt"] == "read sample.txt"
    assert cast(dict[str, object], plan_artifact["metadata"])["request_metadata_keys"] == [
        "provider_stream"
    ]
    plan_steps = cast(list[dict[str, object]], plan_artifact["steps"])
    assert plan_steps == [
        {
            "order": 1,
            "title": "Complete the requested work: read sample.txt",
            "acceptance_criteria": [
                "Address the active user request before finishing execution.",
            ],
            "execution_hints": [
                "Preserve runtime-owned validation and approval boundaries.",
            ],
            "metadata": {
                "agent_preset": "leader",
                "request_metadata_keys": ["provider_stream"],
            },
        }
    ]
    assert _SkillCapturingStubGraph.last_request is not None
    assert _SkillCapturingStubGraph.last_request.metadata["plan_artifact"] == plan_artifact


def test_runtime_direct_execute_skips_plan_creation_and_executes_directly(
    tmp_path: Path,
) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
    ...
    chunks = list(
        runtime.run_stream(
            RuntimeRequest(prompt="read sample.txt", session_id="direct-execute-session")
        )
    )
    response = runtime.resume("direct-execute-session")

    assert response.session.status == "completed"
    assert response.output == "read sample.txt"
    assert all(event.event_type != RUNTIME_PLAN_CREATED for event in response.events)
    assert all(
        chunk.event is None or chunk.event.event_type != RUNTIME_PLAN_CREATED for chunk in chunks
    )
    assert "plan_artifact" not in response.session.metadata
    assert "plan_state" not in response.session.metadata
    assert _SkillCapturingStubGraph.last_request is not None
    assert _SkillCapturingStubGraph.last_request.prompt == "read sample.txt"
    assert "plan_artifact" not in _SkillCapturingStubGraph.last_request.metadata
...
def test_runtime_plan_first_request_received_event_exposes_leader_mode(tmp_path: Path) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
        graph=_SkillCapturingStubGraph(),
        config=RuntimeConfig(
    ...
    assert RUNTIME_PLAN_CREATED in [event.event_type for event in response.events]


def test_runtime_plan_validation_failure_stops_before_graph_execution(tmp_path: Path) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
        graph=_SkillCapturingStubGraph(),
        config=RuntimeConfig(
    ...
    original_build_plan_artifact = runtime_service_module.build_plan_artifact

    def _invalid_plan_artifact(
        *,
        prompt: str,
        metadata: dict[str, object],
        agent_preset: str | None,
    ):
        _ = prompt, metadata, agent_preset
        return PlanArtifact(steps=())

    runtime_service_module.build_plan_artifact = _invalid_plan_artifact
    try:
        response = runtime.run(RuntimeRequest(prompt="read sample.txt", session_id="invalid-plan"))
    finally:
        runtime_service_module.build_plan_artifact = original_build_plan_artifact

    assert response.session.status == "failed"
    assert response.output is None
    assert response.events[-1].event_type == "runtime.failed"
    assert response.events[-1].payload == {
        "error": "runtime plan artifact must include at least one step",
...
def test_runtime_resume_preserves_plan_artifact_and_plan_state_for_waiting_plan_first_session(
    tmp_path: Path,
) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
    ...
    waiting = runtime.run(RuntimeRequest(prompt="go", session_id="plan-first-waiting"))
    replay = runtime.resume("plan-first-waiting")
    effective = runtime.effective_runtime_config(session_id="plan-first-waiting")

    expected_plan_artifact = cast(dict[str, object], waiting.session.metadata["plan_artifact"])
    expected_plan_state = {
        "mode": "plan_first",
        ...
        "current_step_title": "Complete the requested work: go",
        "approval_request_id": cast(str, waiting.events[-1].payload["request_id"]),
        "blocked_tool": "write_file",
    }

    assert waiting.session.status == "waiting"
    assert replay.session.metadata["plan_artifact"] == expected_plan_artifact
    assert replay.session.metadata["plan_state"] == expected_plan_state
...
def test_runtime_plan_first_failure_after_plan_creation_keeps_persisted_truth(
    tmp_path: Path,
) -> None:
    runtime = VoidCodeRuntime(
        workspace=tmp_path,
    ...
    def _invalid_runtime_plan(
        *,
        ...
        _ = prompt, metadata, agent_preset
        return PlanArtifact(
            steps=(PlanStep(order=1, title="Keep plan truth"),),
            acceptance_criteria=("Stay truthful",),
            execution_hints=("Fail after artifact persistence",),
            metadata={"agent_preset": "leader"},
            kind="bad.kind",
        )

    runtime_service_module.build_plan_artifact = _invalid_runtime_plan
    try:
        response = runtime.run(RuntimeRequest(prompt="go", session_id="plan-first-failure"))
    finally:
        runtime_service_module.build_plan_artifact = original_build_plan_artifact

    replay = runtime.resume("plan-first-failure")

    assert response.session.status == "failed"
    assert response.events[-1].payload == {
        "error": "runtime plan artifact kind must be 'leader.plan_first'",
        "kind": "plan_validation_failed",
    }
    assert "plan_artifact" not in replay.session.metadata
    assert "plan_state" not in replay.session.metadata
    runtime_config = cast(dict[str, object], replay.session.metadata["runtime_config"])
...
def test_runtime_plan_first_approval_resume_preserves_plan_context_through_completion(
    tmp_path: Path,
) -> None:
    initial_runtime = VoidCodeRuntime(
        workspace=tmp_path,
    ...
    waiting = initial_runtime.run(RuntimeRequest(prompt="go", session_id="plan-first-resume"))
    approval_request_id = str(waiting.events[-1].payload["request_id"])
    ...
    resumed = resumed_runtime.resume(
        session_id="plan-first-resume",
        approval_request_id=approval_request_id,
        approval_decision="allow",
    )
    replay = resumed_runtime.resume("plan-first-resume")
    plan_artifact = cast(dict[str, object], waiting.session.metadata["plan_artifact"])

    assert resumed.session.status == "completed"
    assert resumed.output == "done"
    assert resumed.session.metadata["plan_artifact"] == plan_artifact
    assert resumed.session.metadata["plan_state"] == {
        "mode": "plan_first",
        "status": "completed",
        "step_count": 1,
        "current_step_index": 0,
    ...
    assert replay.session.metadata["plan_state"] == resumed.session.metadata["plan_state"]


def test_runtime_plan_first_single_agent_approval_wait_preserves_plan_truth(
    tmp_path: Path,
) -> None:
    custom_attempts: list[int] = []
    custom_requests: list[SingleAgentTurnRequest] = []
    registry = ModelProviderRegistry(
        providers={
            "custom": _ApprovalResumeFallbackModelProvider(
                name="custom",
                attempts_seen=custom_attempts,
                requests_seen=custom_requests,
            )
        }
    )
    runtime = VoidCodeRuntime(
    ...
    waiting = runtime.run(
        RuntimeRequest(
            prompt="write alpha.txt 1",
            session_id="plan-first-approval",
        )
    )
    plan_artifact = cast(dict[str, object], waiting.session.metadata["plan_artifact"])
    ...
    assert waiting.session.status == "waiting"
    assert custom_attempts == [0]
    assert len(custom_requests) == 1
    assert custom_requests[0].agent_preset == {
        "preset": "leader",
        "prompt_profile": "leader",
        "model": "custom/demo",
        "execution_engine": "single_agent",
    ...
    assert custom_requests[0].attempt == 0
    assert custom_requests[0].tool_results == ()
    assert plan_artifact["kind"] == "leader.plan_first"
    assert waiting.session.metadata["plan_state"] == {
        "mode": "plan_first",
        "status": "waiting_approval",
        "step_count": 1,
        "current_step_index": 0,
        "current_step_order": 1,
        "current_step_title": "Complete the requested work: write alpha.txt 1",
        "approval_request_id": cast(str, waiting.events[-1].payload["request_id"]),
        "blocked_tool": "write_file",
    }
    assert runtime_config["agent"] == {
...
def test_runtime_plan_first_single_agent_approval_resume_keeps_plan_metadata_and_ownership(
    tmp_path: Path,
) -> None:
    custom_attempts: list[int] = []
    custom_requests: list[SingleAgentTurnRequest] = []
    ...
    waiting = initial_runtime.run(
        RuntimeRequest(prompt="write alpha.txt 1", session_id="plan-first-approval-resume")
    )
    approval_request_id = str(waiting.events[-1].payload["request_id"])
    plan_artifact = cast(dict[str, object], waiting.session.metadata["plan_artifact"])
    ...
    resumed = resumed_runtime.resume(
        session_id="plan-first-approval-resume",
        approval_request_id=approval_request_id,
        approval_decision="allow",
    )
    replay = resumed_runtime.resume("plan-first-approval-resume")

    assert resumed.session.status == "completed"
    assert resumed.output == "done"
    assert custom_attempts == [0, 0, 0]
    assert len(custom_requests) == 3
    assert custom_requests[1].agent_preset == custom_requests[0].agent_preset
    assert custom_requests[1].attempt == 0
    assert custom_requests[1].tool_results == ()
    assert custom_requests[2].agent_preset == custom_requests[0].agent_preset
    assert custom_requests[2].attempt == 0
    assert len(custom_requests[2].tool_results) == 1
    assert custom_requests[2].tool_results[0].tool_name == "write_file"
    assert resumed.session.metadata["plan_artifact"] == plan_artifact
    assert resumed.session.metadata["plan_state"] == {
        "mode": "plan_first",
        "status": "completed",
    ...
    runtime_config = cast(dict[str, object], resumed.session.metadata["runtime_config"])
    assert runtime_config["agent"] == {
        "preset": "leader",
        "prompt_profile": "leader",
        "model": "custom/demo",
    ...
    assert replay.session.metadata["plan_artifact"] == plan_artifact
    assert replay.session.metadata["plan_state"] == resumed.session.metadata["plan_state"]
...
def test_runtime_plan_first_provider_fallback_preserves_plan_truth_and_leader_ownership(
    tmp_path: Path,
) -> None:
    opencode_providers: list[_ScriptedSingleAgentProvider] = []
    custom_providers: list[_ScriptedSingleAgentProvider] = []
    registry = ModelProviderRegistry(
        providers={
            "opencode": _ScriptedModelProvider(
                name="opencode",
                ...
                created_providers=opencode_providers,
            ),
            "custom": _ScriptedModelProvider(
                name="custom",
                outcomes=(SingleAgentTurnResult(output="fallback complete"),),
                created_providers=custom_providers,
            ),
        }
    )
    runtime = VoidCodeRuntime(
    ...
    response = runtime.run(
        RuntimeRequest(prompt="read sample.txt", session_id="plan-first-provider-fallback")
    )
    runtime_config = cast(dict[str, object], response.session.metadata["runtime_config"])
    plan_artifact = cast(dict[str, object], response.session.metadata["plan_artifact"])
    fallback_events = [
    ...
    assert response.session.status == "completed"
    assert response.output == "fallback complete"
    assert response.session.metadata["provider_attempt"] == 1
    assert len(fallback_events) == 1
    assert fallback_events[0].payload == {
        "reason": "rate_limit",
        "from_provider": "opencode",
    ...
    assert plan_artifact["kind"] == "leader.plan_first"
    assert response.session.metadata["plan_state"] == {
        "mode": "plan_first",
        "status": "completed",
        "step_count": 1,
        "current_step_index": 0,
        "current_step_order": 1,
        "current_step_title": "Complete the requested work: read sample.txt",
    }
    assert runtime_config["agent"] == {
        "preset": "leader",
        "prompt_profile": "leader",
    ...
    assert len(opencode_providers) == 1
    assert len(custom_providers) == 1
    assert opencode_providers[0].requests[0].agent_preset == {
        "preset": "leader",
        "prompt_profile": "leader",
        "model": "opencode/gpt-5.4",
        "execution_engine": "single_agent",
    ...
    assert opencode_providers[0].requests[0].attempt == 0
    assert custom_providers[0].requests[0].agent_preset == {
        "preset": "leader",
        "prompt_profile": "leader",
        "model": "opencode/gpt-5.4",
        "execution_engine": "single_agent",
    ...
    assert custom_providers[0].requests[0].attempt == 1
