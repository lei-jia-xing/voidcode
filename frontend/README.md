...
> **重要提示：** 目前的前端主要仍是一个 **UI 壳程序**。不过仓库现在已经具备最小可用的运行时传输路径：会话列表、会话重放以及流式运行请求可以通过本地 HTTP/SSE 后端获取，而更完整的 Web 交互体验仍在继续演进。

- [x] UI 壳程序与导航
- [x] 运行时驱动的会话列表 / 会话重放 / 流式运行基础路径
- [ ] 更完整的运行时驱动任务体验
- [ ] WebSocket 事件流（计划中）
...
**注意：** 这些接口目前仍是 **MVP 级别的部分实现**。仓库已经具备真实的运行时传输客户端和本地后端服务器路径，当前 UI 可以消费会话列表、重放数据和流式运行事件，但更完整的产品化 Web 体验仍未完成。
