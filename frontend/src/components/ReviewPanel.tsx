...
const DEFAULT_FILE_TREE_PANEL_WIDTH = 280;
const MIN_PANEL_WIDTH = 384;
const MIN_FILE_TREE_PANEL_WIDTH = 244;
const MAX_PANEL_WIDTH = 960;
const MAX_FILE_TREE_PANEL_WIDTH = 420;
const MIN_MAIN_CONTENT_WIDTH = 280;

type ReviewSurface = "code-review" | "file-tree";
...
export function ReviewPanel({
  ...
  const isFileTreeSurface = surface === "file-tree";
  const defaultPanelWidth = isFileTreeSurface
    ? DEFAULT_FILE_TREE_PANEL_WIDTH
    : DEFAULT_PANEL_WIDTH;
  const minPanelWidth = isFileTreeSurface
    ? MIN_FILE_TREE_PANEL_WIDTH
    : MIN_PANEL_WIDTH;
  const maxPanelWidth = isFileTreeSurface
    ? MAX_FILE_TREE_PANEL_WIDTH
    : MAX_PANEL_WIDTH;
  ...
  const resizeToClientX = useCallback(
    (clientX: number) => {
      ...
      const maxWidth = Math.max(
        minPanelWidth,
        Math.min(maxPanelWidth, availableWidth - MIN_MAIN_CONTENT_WIDTH),
      );
      const nextWidth = Math.min(
        maxWidth,
        Math.max(minPanelWidth, availableWidth - clientX),
      );
  ...
  const title = isFileTreeSurface
    ? t("review.fileTree")
    : t("review.codeReview");
  const closeLabel = isFileTreeSurface
    ? t("review.closeFileTree")
    : t("review.closeCodeReview");
  const resizeLabel = isFileTreeSurface
    ? t("review.resizeFileTree")
    : t("review.resizeCodeReview");
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-[color:var(--vc-border-subtle)] px-4 py-3">
        <ControlButton
          compact
          variant="ghost"
          onClick={onRefresh}
                  ...
                  {snapshot.changed_files.map((item) => {
                    ...
                    return (
                      <button
                        key={item.path}
                        type="button"
                        aria-label={`${changeLabel(item.change_type)} ${item.path}`}
