...
import { ChevronDown, ChevronRight, CheckCircle2, XCircle, Loader2, Wrench, PauseCircle, AlertCircle, User, Bot } from 'lucide-react';
  ...
  return (
    <div className="mb-3">
      ...
      >
        ...
        <span className="text-slate-600">({thinking.length} {thinking.length === 1 ? 'step' : 'steps'})</span>
      </button>
      {expanded && (
...
function ToolIndicators({ tools }: { tools: { name: string; status: string }[] }) {
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-300">
              {t('approval.message', { target: approval.targetSummary || approval.tool || t('approval.unknownTarget') })}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        {!hasMessages && (
          <div className="flex flex-col items-center justify-center py-20 text-slate-500">
            ...
            <p className="text-lg font-medium text-slate-400 mb-1">{t('chat.welcomeTitle')}</p>
            ...
            <div key={message.id} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-slate-300">{t('chat.assistantName')}</span>
