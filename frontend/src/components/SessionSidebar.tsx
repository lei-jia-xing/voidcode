...
import { Code2, Globe, Settings, Server, Loader2, CheckCircle2, XCircle, Plus } from 'lucide-react';
    ...
    <aside className="w-16 md:w-64 border-r border-slate-800 bg-[#09090b] flex flex-col justify-between flex-shrink-0">
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        <div className="p-3 flex-1 overflow-y-auto hidden md:block">
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 px-2">{t('session.listHeader')}</div>
              ...
              >
                <div className="w-full flex items-center justify-between gap-2 mb-1">
                  <div className="flex items-center truncate max-w-[75%]">
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 mr-2 ${getSessionStatusDotClass(s.status)}`} />
                    <span className="font-medium text-sm truncate">
                      {s.prompt || <span className="font-mono">{s.session.id.substring(0, 8)}</span>}
                    </span>
                  </div>
                  <span className={`flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-md font-medium border ${getStatusColorClass(s.status)}`}>
          ...
          {sessionsStatus === 'loading' && (
            <p className="mt-3 px-2 text-xs text-slate-500">{t('session.loadingList')}</p>
          )}
          {sessionsError && (
            <p className="mt-3 px-2 text-xs text-rose-400">{t('session.loadError', { message: sessionsError })}</p>
          )}
        </div>
      </div>
      ...
      <div className="p-3 border-t border-slate-800 space-y-1">
        ...
        >
          ...
          <span className="hidden md:block font-medium text-sm">{language === 'en' ? t('language.zh') : t('language.en')}</span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span className="hidden md:block font-medium text-sm">
            {runtimeTestStatus === 'testing' ? t('debug.testing') :
             runtimeTestStatus === 'success' ? t('debug.success') :
             runtimeTestStatus === 'error' ? t('debug.error') :
             t('debug.testRuntime')}
          </span>
        </button>
        <button
        ...
        >
          ...
          <span className="hidden md:block font-medium text-sm">{t('nav.settings')}</span>
        </button>
      </div>
    </aside>
  );
