...
async function installMockRuntime(page: Page) {
  const workspaceSnapshot = {
    current: { path: '/workspace', label: 'workspace', available: true, current: true, last_opened_at: 1 },
    recent: [{ path: '/workspace', label: 'workspace', available: true, current: true, last_opened_at: 1 }],
    candidates: [{ path: '/tmp/voidcode-demo', label: 'voidcode-demo', available: true, current: false }],
  };
  const statusSnapshot = {
    git: { state: 'git_ready', root: '/workspace', error: null },
    lsp: { state: 'running', error: null, details: {} },
    mcp: { state: 'stopped', error: null, details: { retry_available: true, servers: [{ server: 'local', status: 'stopped', stage: 'idle' }] } },
    acp: { state: 'running', error: null, details: { status: 'connected', last_request_type: 'handshake' } },
  };
  ...
  const approvalSession = { session: { id: 'browser-session' }, status: 'waiting', turn: 1, metadata: {} };
  const completedSession = { session: { id: 'browser-session' }, status: 'completed', turn: 1, metadata: {} };
  const requestEvent = {
  ...
  const readEvent = {
    ...
    payload: {
      ...
      tool_status: {
        ...
        tool_name: 'read_file',
        status: 'completed',
        display: { kind: 'context', title: 'Read', summary: 'Read README.md', args: ['README.md'] },
      },
    },
  };
  const grepEvent = {
    ...
    payload: {
      ...
      tool_status: {
        ...
        tool_name: 'grep',
        status: 'completed',
        display: { kind: 'context', title: 'Search', summary: 'Search TODO', args: ['TODO', '.'] },
      },
    },
  };
  const shellEvent = {
    ...
    payload: {
      ...
      data: { command: 'bun run --cwd frontend lint', exit_code: 0, stdout: 'lint passed', stderr: '' },
      tool_status: {
        ...
        display: {
          ...
          copyable: { command: 'bun run --cwd frontend lint', output: 'lint passed' },
        },
      },
    },
  };
  const approvalEvent = {
    ...
    source: 'runtime',
    payload: { request_id: 'approval-1', tool: 'write_file', target_summary: 'write note.txt' },
  };
  const responseEvent = {
    session_id: 'browser-session',
    sequence: 6,
  ...
  await page.route('**/api/workspaces', async (route) => route.fulfill(jsonResponse(workspaceSnapshot)));
  await page.route('**/api/workspaces/open', async (route) => route.fulfill(jsonResponse(workspaceSnapshot)));
  await page.route('**/api/sessions', async (route) => route.fulfill(jsonResponse([{ session: { id: 'browser-session' }, status: 'waiting', turn: 1, prompt: 'browser QA tool surfaces', updated_at: 1 }])));
  await page.route('**/api/sessions/browser-session', async (route) => route.fulfill(jsonResponse({ session: approvalSession, events: [requestEvent, readEvent, grepEvent, shellEvent, approvalEvent], output: null })));
  await page.route('**/api/sessions/browser-session/debug', async (route) => route.fulfill(jsonResponse({ session: approvalSession, prompt: 'browser QA tool surfaces', persisted_status: 'waiting', current_status: 'waiting', active: false, resumable: true, replayable: true, terminal: false, pending_question: null, failure: null, last_tool: 'shell_exec', suggested_operator_action: 'approve', operator_guidance: 'Resolve approval.' })));
  await page.route('**/api/sessions/browser-session/tasks', async (route) => route.fulfill(jsonResponse([])));
  await page.route('**/api/providers', async (route) => route.fulfill(jsonResponse([{ name: 'opencode-go', label: 'OpenCode Go', configured: true, current: true }])));
  await page.route('**/api/providers/opencode-go/models', async (route) => route.fulfill(jsonResponse({ provider: 'opencode-go', configured: true, models: ['glm-5.1'], source: null, last_refresh_status: 'ok', last_error: null, discovery_mode: 'configured_endpoint', model_metadata: { 'glm-5.1': { context_window: 128000, max_output_tokens: 8192, supports_reasoning_effort: true, default_reasoning_effort: 'medium' } } })));
  await page.route('**/api/providers/opencode-go/validate', async (route) => route.fulfill(jsonResponse({ provider: 'opencode-go', configured: true, ok: true, status: 'ok', message: 'Provider credentials valid.' })));
  await page.route('**/api/agents', async (route) => route.fulfill(jsonResponse([{ id: 'leader', label: 'Leader', description: null, selectable: true }])));
  ...
  await page.route('**/api/runtime/run/stream', async (route) => route.fulfill({
    status: 200,
    contentType: 'text/event-stream',
    body: sseBody([
      { kind: 'event', session: approvalSession, event: requestEvent, output: null },
      { kind: 'event', session: approvalSession, event: readEvent, output: null },
      { kind: 'event', session: approvalSession, event: grepEvent, output: null },
      { kind: 'event', session: approvalSession, event: shellEvent, output: null },
      { kind: 'event', session: approvalSession, event: approvalEvent, output: null },
    ]),
  }));
  await page.route('**/api/sessions/browser-session/approval', async (route) => {
    const body = route.request().postDataJSON() as { request_id?: string; decision?: string };
    expect(body).toMatchObject({ request_id: 'approval-1', decision: 'allow' });
    await route.fulfill(jsonResponse({ session: completedSession, events: [requestEvent, readEvent, grepEvent, shellEvent, approvalEvent, responseEvent], output: 'Browser QA completed.' }));
  });
}
...
test.describe('VoidCode Web Launcher', () => {
  ...
  test('should cover browser UI controls and monochrome tool layout', async ({ page }) => {
    ...
    await page.goto(launcherUrl);
    await expect(page.getByRole('button', { name: 'Model' })).toHaveText(/OpenCode Go/);
    await expect(page.getByText('workspace', { exact: true })).toBeVisible();

    const resizeHandle = page.getByRole('separator', { name: 'Resize session sidebar' });
    ...
    const shellToggle = page.getByRole('button', { name: /show details for shell/i });
    ...
    await expect.poll(() => page.evaluate(() => (window as unknown as { __copied?: string[] }).__copied ?? [])).toContain('bun run --cwd frontend lint');
    await page.screenshot({ path: path.join(evidenceDir, 'task-11-tool-browser.png'), fullPage: true });
    ...
    await page.getByPlaceholder('Search recent or nearby projects').fill('demo');
    await expect(page.getByText('voidcode-demo', { exact: true })).toBeVisible();
  ...
  test('should show the project-first empty state in the launcher shell', async ({ page }) => {
    await page.addInitScript(() => window.localStorage.clear());
    await page.route('**/api/workspaces', async (route) => route.fulfill(jsonResponse({ current: null, recent: [], candidates: [] })));
    await page.route('**/api/providers', async (route) => route.fulfill(jsonResponse([])));
    await page.route('**/api/agents', async (route) => route.fulfill(jsonResponse([])));
    await page.route('**/api/status', async (route) => route.fulfill(jsonResponse({ git: { state: 'not_git_repo', root: null, error: null }, lsp: { state: 'stopped', error: null, details: {} }, mcp: { state: 'stopped', error: null, details: {} }, acp: { state: 'stopped', error: null, details: {} } })));
    await page.route('**/api/review', async (route) => route.fulfill(jsonResponse({ root: null, git: { state: 'not_git_repo', root: null, error: null }, changed_files: [], tree: [] })));
    await page.route('**/api/settings', async (route) => route.fulfill(jsonResponse({ provider: null, model: null, provider_api_key_present: false })));
    await page.route('**/api/sessions', async (route) => route.fulfill(jsonResponse([])));
    await page.route('**/api/notifications', async (route) => route.fulfill(jsonResponse([])));
    await page.route('**/api/tasks', async (route) => route.fulfill(jsonResponse([])));

    await page.goto(launcherUrl);
    await expect(page.getByText('Open a project to get started')).toBeVisible();
    await expect(page.getByText('Choose a workspace first before using chat, review, or composer.')).toBeVisible();
