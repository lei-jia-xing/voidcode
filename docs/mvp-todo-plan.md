...
目前的 MVP 主执行计划已经基本收口：运行时、终端客户端和真实 Web 客户端之间的核心集成闭环已被实现并验证。
...
- [x] 将流式运行时事件渲染为可滚动、可读的时间线
- [x] 在活动提要中实现可折叠/可检查的工具调用和结果块
- [x] 为高风险工具执行集成交互式审批提示
- [x] 直接在 TUI 中实现会话管理（列表、加载、恢复）
- [x] 为 `docs/tui-mvp-spec.md` 中定义的规范 TUI 冒烟流程添加自动化冒烟测试
...
- [x] 定义一个覆盖 运行时 + TUI + Web 客户端 的规范演示流程
- [x] 为主要产品循环添加全栈测试覆盖
- [ ] 针对常见失败和恢复记录操作员工作流
- [ ] 添加调试实时会话所需的观测钩子/日志
