...
- 通知的去重与重启恢复语义
- CLI / HTTP 的 task status、output、cancel、list parity
...
- 通过 CLI 与 HTTP 暴露 task status / output / cancel / list surfaces
...
- MCP marketplace、provider marketplace 或 dynamic agents marketplace
...
- retry guidance 是显式的，不能引入无限自动重试
...
- 当前顶层 active preset 是 `leader` 和显式 `product`；delegated child presets 是 `advisor`、`explore`、`product`、`researcher`、`worker`。
...
- 当前不声明 workspace-scoped MCP lifecycle、MCP marketplace、dynamic MCP install flow 或 skill marketplace。
...
8. fake provider 与 fake MCP 测试覆盖 routing、tool guardrail、skill force-load、MCP lifecycle、result、retry 和 cancel paths。
