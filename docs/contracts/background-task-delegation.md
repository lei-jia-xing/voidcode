...
- `background_process(op="start")` 中的 live process reuse 仍然以精确匹配为准，只有 trimmed command 相同且 workspace 相同才算同一个可复用进程
- `background_process(op="logs")` 提供的是有界的日志状态检查，读取的是 retained stdout/stderr，不是持续 watch 或订阅流
