...
- `providers`：provider 级配置对象；当未提供仓库本地 `providers` block 时，runtime 也会从标准 provider 凭据环境变量构造最小 provider 配置（例如 `OPENCODE_GO_API_KEY`）
...
- `preset`
- `prompt_profile`：注入 provider-backed single-agent turn 的 agent profile system message
- `model`：覆盖本次运行的 resolved provider model
- `execution_engine`：`leader` 默认进入 provider-backed `single_agent` 路径
...
- `worker` / `advisor` / `explore` / `researcher` / `product` 的 manifest fields：仍是 future preset intent，不能通过 active runtime config 触发真实执行 agent。
