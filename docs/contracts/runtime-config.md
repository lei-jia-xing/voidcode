...
- `execution_engine`：当前接受 `deterministic` 或 `provider`。`deterministic` 仍是兼容默认值，用于无凭据本地演示、测试和参考/debug harness；真实产品主路径应在配置了 `model` / provider 后使用 `provider`。
...
- `agent`：agent preset 的 runtime 消费入口。当前顶层 active run 只接受 `leader`；runtime-owned delegation path 上的 child run 还可执行 `advisor`、`explore`、`product`、`researcher`、`worker`。

### Execution engine 生命周期决策

VoidCode 暂不把顶层默认值直接切换为 `provider`。原因是 CI、本地 contributor workflow 和 `uv run voidcode run "read README.md"` 仍需要在没有任何 provider 凭据时保持可用；`provider` engine 在实际执行时必须能够解析出 model/provider target。因此当前生命周期约定为：

- `deterministic`：保留为显式支持的兼容 / 参考 / debug engine，并继续承担无 key 本地 smoke test、graph harness 和确定性回归测试。
- `provider`：作为新产品能力和真实 agent 行为的主推荐路径；当配置 `execution_engine = "provider"` 与 `model = "provider/model"` 时，runtime 进入 provider-backed execution。
- 未来若要翻转顶层默认值，应同时提供默认 model 迁移策略、CI 无凭据路径和面向用户的配置迁移说明。
...
- runtime-owned delegation path 上的 child run 还可执行 `advisor`、`explore`、`product`、`researcher`、`worker`

这个限制是有意的：这些 child preset 不是任意可选的顶层 active agent，只有在 delegation path（例如带 `parent_session_id` 且通过 subagent routing 校验的请求）中才会进入真实执行路径。
...
- `execution_engine`：`leader` 默认进入 runtime-managed `provider` 路径
...
以下字段当前仍只作为 parsed config intent 保留，不代表 runtime 已经实现相应能力语义：

- `agent.tools.paths`：目前没有自定义 tool loader；不会加载额外工具。
- `AgentManifest.routing_hints`：仍属于声明层 metadata，不是执行治理 truth。
- 把 `worker` / `advisor` / `explore` / `researcher` / `product` 作为任意顶层 active preset 的 config intent：runtime 仍不会把它们当作普通顶层会话直接执行。

如果运行时在顶层 active run 中收到非 `leader` preset，例如：
