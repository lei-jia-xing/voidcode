...
- `tools`：内置工具启用与 provider 可见工具收窄的最小配置；当前不包含自定义 tool loader
...
当前 provider 路径缺少同 target 重试预算：

- `src/voidcode/provider/litellm_backend.py` 的 sync 与 stream 路径都显式传入 `num_retries: 0`，禁用了 LiteLLM 层可用的重试入口
- `src/voidcode/runtime/run_loop.py` 的 provider error 处理只对跨 target fallback 有逻辑；当 `fallback_selection is None` 时，即使是 `transient_failure` 或 `rate_limit` 等被分类为可恢复的错误，也会直接 emit `runtime.failed` 并标记 `fallback_exhausted: true`
- 单 target + 无 fallback chain 场景下，一次瞬时网络抖动（SSL 错误、stream chunk 超时等）会在 attempt 1 就终结整个会话
- `format_fallback_exhausted_error` 在 `"fallback exhausted ... attempt 1"` 的消息在这种情况下具有误导性

变通方案：配置 `provider_fallback` 至少一个 fallback model 以启用跨 target 故障转移。同 target retry budget（带指数退避 + jitter）是计划中的增强，预期配置形状类似：
...
实现后将 emit `runtime.provider_transient_retry` 事件，仅在耗尽同 target 重试预算后才进入 `provider_fallback` chain。
- `agent`：agent preset 的 runtime 消费入口。当前顶层 active run 默认使用 `leader`，也可显式选择 `product` 进入规划/需求模式；runtime-owned delegation path 上的 child run 还可执行 `advisor`、`explore`、`product`、`researcher`、`worker`。
- `agents`：按 agent preset 配置 model / fallback defaults；内置 preset key 可省略 `preset`，自定义 key 必须声明 `preset`。
...
- `agents.<preset>`：按 preset 配置 delegated child / primary agent defaults；内置 key 可省略 `preset`，自定义 key 必须显式声明 `preset`。
...
- 顶层 active run 默认使用 `leader`，也可显式选择 `product`
