...
- `command` 由 runtime 在 workspace cwd 中执行；tool arguments 以 JSON 写入 stdin，同时也通过 `VOIDCODE_TOOL_ARGUMENTS` 环境变量提供。
- runtime 注入 `VOIDCODE_WORKSPACE`、`VOIDCODE_TOOL_NAME`、`VOIDCODE_TOOL_CALL_ID`（如有）、`VOIDCODE_SESSION_ID`、`VOIDCODE_PARENT_SESSION_ID`（如有）和 `VOIDCODE_DELEGATION_DEPTH`，tool 作者不应从客户端获得这些上下文字段。
