...
对于 hooks/config MVP，`approval_mode` 和 `model` 的锁定优先级为：
...
这有意设计得很窄：在 MVP 中，只有 `approval_mode` 和 `model` 是会话可覆盖且恢复关键的字段。
...
- 除 `approval_mode` / `model` 外，其余扩展领域继续保持浅层仓库本地配置
- 仅 `approval_mode` / `model` 在此轨道中具备完整优先级行为
