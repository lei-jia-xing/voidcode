...
`server` 可省略。`line` 与 `character` 是 1-based；`textDocument/diagnostic` 与 `textDocument/documentSymbol` 不需要位置参数，`workspace/symbol` 额外接受 `query`。
...
- 选择原则：
  - 已知符号名但不知道位置时，先用 `workspace/symbol`。
  - 需要文件内结构或先找函数/类时，用 `textDocument/documentSymbol`。
  - 已知精确位置时，再用 definition / references / hover / implementation / call hierarchy。
  - `incomingCalls` / `outgoingCalls` 内部会先做 `prepareCallHierarchy`，agent 不需要自己拆成两步。
  - 纯文本搜索、关键字匹配、批量模式查找不要用 LSP。
...
- 选择原则：需要 diagnostics、定义、引用、hover、symbol、call hierarchy 等 language-server 语义时使用；纯文本搜索不要用 LSP。
