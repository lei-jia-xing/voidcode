...
- 外部目录访问（规划中，#338）：当 `external_directory_read` / `external_directory_write` permission 配置后，tool call 可在受控条件下访问 workspace 外部路径。所有路径必须先 canonicalize/realpath，symlink escape 按真实路径判定。approval prompt 会显示外部绝对路径与操作类型。tool result 中的外部路径以 absolute path 展示，不伪装为 `../../..` 相对路径。
...
图片或 PDF 会把 `data.type` 设为 `attachment`，并通过 `data.attachment` / `attachment` 返回 base64 data URI。目录路径会失败；目录探索应使用 `list` 或 `glob`。
