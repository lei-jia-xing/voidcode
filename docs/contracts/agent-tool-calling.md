...
- 分组：workspace read
- 只读：是
- 用途：读取 workspace 内文件；UTF-8 文本会以内联内容返回，图片与 PDF 以附件数据返回。
- 参数：
...
- 分组：workspace search
- 只读：是
- 用途：在 workspace 内的文件或目录目标上做 literal 或 regex 文本搜索。
- 参数：
...
`regex`、`context`、`include` 与 `exclude` 都可省略。`path` 可指向单个文件，也可指向目录。

- 成功返回：

```json
{
  "content": "Found 2 match(es) for 'ToolCall' in src\nsrc/example.py:10: ...",
  "data": {
    "path": "src",
    "pattern": "ToolCall",
    "regex": false,
...
- `diff` 是面向客户端工具活动视图的 unified diff 预览；当 formatter 修改写入内容时，它反映 formatter 后落盘内容与写入前内容的差异。
...
```json
{"command": "pytest tests/unit", "timeout": 600}
```

`timeout` 可省略，默认 120 秒，最大 600 秒。
...
| 创建或完整重写文件 | `write_file` | 不要用 patch 表达全量新内容，除非需要 diff 审阅；不要先建空文件再用 shell 填内容 |
| 执行测试 / 构建 | `shell_exec` | 不要用 shell 做已有窄工具可完成的读取/写入操作 |
...
2. **选择最窄工具。** 能用 `read_file` 就不要用 `shell_exec cat`；能用 `edit` 就不要完整 `write_file`；已知内容写文件时不要用 `shell_exec` 的重定向、heredoc 或 `cat <<EOF`。
...
8. **后台任务依赖 runtime 通知。** 启动后台任务后，不要 sleep、紧轮询或重复启动等价任务来“确认进度”；优先等待 runtime completion reminder，再通过 `background_output` 或专门结果面读取状态/结果。
9. **shell file authoring 会被拒绝。** 对于已知内容的文件创建/覆盖，runtime 应拒绝 `shell_exec` 中基于 `echo >`、`printf >`、`cat <<EOF` 等模式的 authored write，并要求 agent 回到 `write_file` / `edit` / `multi_edit`。
