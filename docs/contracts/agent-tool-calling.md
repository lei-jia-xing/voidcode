...
| `docs/contracts/runtime-events.md` | 客户端渲染的稳定事件词汇 | 本文档引用 tool lookup / permission / completed 等事件，不重新定义完整事件表 |
...
4. 调用工具实现；
5. 发出 tool result 事件并运行 post-tool hooks；
6. 将 session、events、approval state 和 checkpoint 持久化。
...
在 runtime event 流中，成功执行后的结果以 `runtime.tool_completed` 发出，payload 至少包含：
...
- runtime hooks 可能在工具执行前后发出额外事件；agent 不应假设 tool call 与 tool result 之间只有一个事件。
...
5. 执行成功后，runtime 发出 `runtime.tool_completed`，payload 中包含 diff、additions、deletions 等结构化结果。
