...
- [x] **Runtime Harness Policy v1 基线**：运行时已 materialize/persist `RuntimePolicySnapshot` v1，并覆盖固定 precedence、neutral intent metadata、hook non-authority、product non-delegation、legacy synthesis、prompt activation persistence/replay 以及 bounded `runtime.policy_materialized` observability；它仍不是通用 policy DSL 或任意多智能体平台。
