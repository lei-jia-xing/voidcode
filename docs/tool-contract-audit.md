...
审计范围：工具定义、参数 schema、`ToolResult` 展示约定、运行时权限元数据，以及 CLI/命令 shorthand。重点检查 `read_file` 的 XML-like 输出和 `grep` 的正则语义。
...
- `read_file` 的 `offset`/`limit` 默认值、`next_offset` 与续读提示在实现和契约文档之间一致。
...
2. 统一 `ToolResult` 的摘要/正文边界，逐步淘汰 `read_file` XML-like 包装。
