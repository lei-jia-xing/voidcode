...
- 在可行的情况下保持函数短小且确定性。
- Runtime 新行为应遵循 [`runtime-architecture-refactor-plan.md`](./runtime-architecture-refactor-plan.md)：只实现当前版本契约，不接受已删除 metadata key，也不让客户端/graph/tool 复制 runtime 治理状态。
