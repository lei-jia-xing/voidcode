...
- 当前 graph 层仍保留一个小型的确定性 reference engine。
...
当前 deterministic graph/reference engine 更像一个“参考实现”。WIP 之后，运行时应显式支持 engine selection。

建议抽象：

- `deterministic`：当前 deterministic parser/graph engine
- `provider`：新的 provider-backed execution engine
