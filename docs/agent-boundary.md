...
4. 顶层 active run 默认使用 `leader`（`product` 已不再作为顶层 preset 直接选择）；runtime-owned delegation path 上的 child run 可执行 `advisor`、`explore`、`researcher`、`worker`、`product`（`plan`/`planning` 类别路由到 product）。
