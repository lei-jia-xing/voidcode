...
**当前状态：** 基本完成。运行时事件模式已稳定，并为所有主要循环阶段发出事件。已实现轮次重新编号，以确保会话恢复时的序列一致性。钩子注册基础设施存在于配置模型中，但实时钩子执行仍在待办中。
...
**当前状态：** 进行中。CLI 支持基础任务执行和审批。计划开发 TUI MVP。Web 客户端目前由 mock 数据驱动，但已准备好进行集成。
